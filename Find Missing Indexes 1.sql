...
SELECT  
	[ReadableImpact]=replace(convert(varchar,cast((avg_total_user_cost * avg_user_impact) * (user_seeks + user_scans) as money),1), '.00',''),
...
ORDER BY (avg_total_user_cost * avg_user_impact) * (user_seeks + user_scans) DESC , [CreateIndexStatement] DESC
